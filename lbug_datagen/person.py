...
def generate_persons(cfg: DatagenConfig, d: Dictionaries, city_ids, n_tags: int,
    ...
    return {
        "Person": person,
        "Person_isLocatedIn_City": pa.table({"FROM": located["FROM"], "TO": located["TO"]}),
        "Person_studyAt_University": pa.table({"FROM": study_from, "TO": study_to,
                             "classYear": pa.array(study_year, type=pa.int64())}),
        "Person_workAt_Company": pa.table({"FROM": work_from, "TO": work_to,
                            "workFrom": pa.array(work_since, type=pa.int64())}),
        "Person_hasInterest_Tag": pa.table({"FROM": int_from, "TO": int_to}),
